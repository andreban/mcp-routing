...
async fn greet(
    ...
    Ok(format!(
        "Hello, {}! (Request #{count}, Client: {client_name})",
        params.name
    ))
}
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let greet_tool = Tool {
        ...
        description: Some("Greets a user with request statistics".to_string()),
        input_schema: json!({
            "type": "object",
            "properties": {
    ...
    let mcp_router = McpRouter::new(server_info)
        .instructions("MCP server demonstrating extractors and state sharing")
